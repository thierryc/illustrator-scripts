...
// Version 2.1.b15 Adds PSD exports and iOS and android devices automation (by thierryc(at)guthub.com)
//
// Exports Illustrator artboards and/or layers as individual PNG, EPS, SVG, PSD or PDF files
// 
...
getPsdOptions = function ( transparency, scaling, embedImage, embedFont, trimEdges ) {
...
savePng8 = function ( doc, filePath, options, artboardIndex, artboardName , scaling, densitySuffix) {
	var destFile = new File( filePath.path + '/' + filePath.prefix + filePath.name + filePath.suffix + '.png' );
	doc.exportFile(destFile, ExportType.PNG8 , options);
}
savePng24 = function ( doc, filePath, options, artboardIndex, artboardName , scaling, densitySuffix) {
	var destFile = new File( filePath.path + '/' + filePath.prefix + filePath.name + filePath.suffix + '.png' );
	doc.exportFile(destFile, ExportType.PNG24 , options);
}
savePdf = function ( doc, filePath, options, artboardIndex, artboardName , scaling, densitySuffix) {
	var destFile = new File( filePath.path + '/' + filePath.prefix + filePath.name + filePath.suffix + '.pdf' );   
	options.artboardRange = (artboardIndex+1).toString();
	doc.saveAs( destFile, options, artboardIndex, artboardName);
}
saveJpg = function ( doc, filePath, options, artboardIndex, artboardName , scaling, densitySuffix) {
	var destFile = new File( filePath.path + '/' + filePath.prefix + filePath.name + filePath.suffix + '.jpg' );
	doc.exportFile(destFile, ExportType.JPEG , options);
}
saveEps = function ( doc, filePath, options, artboardIndex, artboardName , scaling, densitySuffix) {
	var destFile = new File( filePath.path + '/' + filePath.prefix + filePath.name + filePath.suffix + '.eps' );
	options.artboardRange = (artboardIndex+1).toString();
	doc.saveAs( destFile, options, artboardIndex, artboardName);		
}
saveSvg = function ( doc, filePath, options, artboardIndex, artboardName , scaling, densitySuffix) {
	var destFile = new File( filePath.path + '/' + filePath.prefix + filePath.name + filePath.suffix + '.svg' );
	doc.exportFile(destFile, ExportType.SVG , options);
}
saveFxg = function ( doc, filePath, options, artboardIndex, artboardName , scaling, densitySuffix) {
	var destFile = new File( filePath.path + '/' + filePath.prefix + filePath.name + filePath.suffix + '.fxg' );
	options.artboardRange = (artboardIndex+1).toString();
	doc.saveAs(destFile, options, artboardIndex, artboardName);
}
savePsd = function ( doc, filePath, options, artboardIndex, artboardName, scaling, densitySuffix) {
	
	var documentName = multi_exporter.slug(doc.name.replace(".ai",""));
	var destFile = new File(filePath.path + '/' + filePath.prefix + documentName + '.psd' );
	var dpi = parseFloat(Math.round(scaling * 0.72));
	var artboardsName = doc.artboards[artboardIndex].name;
	
	if (densitySuffix) {
	  doc.artboards[artboardIndex].name = multi_exporter.slug(artboardsName) + filePath.suffix + densitySuffix;
	} else {
	  doc.artboards[artboardIndex].name = multi_exporter.slug(artboardsName) + filePath.suffix;
	}
	
	options.saveMultipleArtboards = true;
	options.artboardRange = (artboardIndex+1).toString();
  options.resolution = dpi;
	//doc.rasterEffectSettings.resolution = options.resolution;
  //doc.rasterEffectSettings.antiAliasing = true;
  doc.rasterEffectSettings.transparency = true;
  
	doc.exportFile(destFile, ExportType.PHOTOSHOP, options);
	
	doc.artboards[artboardIndex].name = artboardsName;
}
...
var multi_exporter = {
  ...
  iosScalingCheckBox:null, // 200
  ios2xScalingCheckBox:null, // 200
  mdpiScalingCheckBox:null, // 100
  hdpiScalingCheckBox:null, // 150
  xhdpiScalingCheckBox:null, // 225
  
  iosScaling:false, // 100
  ios2xScaling:false, // 200
  mdpiScaling:false, // 100
  hdpiScaling:false, // 150
  xhdpiScaling:false, // 225
  
  _iosScaling:100, // 100
  _ios2xScaling:200, // 200
  _mdpiScaling:100, // 100
  _hdpiScaling:150, // 150
  _xhdpiScaling:225, // 225
	...
	formatInfo: [   {name:"PNG 8", copyBehaviour:false, getOptions:getPng8Options, saveFile:savePng8, activeControls:["scalingInput","transCheckBox","trimEdgesCheckBox","skipDefaultNamesCheckBox"]},
					...
					{name:"PSD", copyBehaviour:false, getOptions:getPsdOptions, saveFile:savePsd, activeControls:["scalingInput","trimEdgesCheckBox","skipDefaultNamesCheckBox"]}],
	...
	load_prefs: function() {
      ...
      /* 
      saved_data.appendChild( new XML('<nyt_iosScaling>false</nyt_iosScaling>') );
      saved_data.appendChild( new XML('<nyt_mdpiScaling>false</nyt_mdpiScaling>') );
      saved_data.appendChild( new XML('<nyt_hdpiScaling>false</nyt_hdpiScaling>') );
      saved_data.appendChild( new XML('<nyt_xhdpiScaling>false</nyt_xhdpiScaling>') );
      */
        ...
        this.skipDefaultNames   = this.prefs_xml.nyt_skipDefaultNames == "true" ? true : false;
				...
				this.iosScaling	        = this.prefs_xml.nyt_iosScaling == "true" ? true : false;
				this.ios2xScaling	        = this.prefs_xml.nyt_ios2xScaling == "true" ? true : false;
				this.mdpiScaling	        = this.prefs_xml.nyt_mdpiScaling == "true" ? true : false;
				this.hdpiScaling	        = this.prefs_xml.nyt_hdpiScaling == "true" ? true : false;
				this.xhdpiScaling	        = this.prefs_xml.nyt_xhdpiScaling == "true" ? true : false;
	...
	show_dialog: function() {
		...
		var mobileLabel = row.add('statictext', undefined, 'iOS/Android:'); 
		mobileLabel.size = [100,20]
		
		this.iosScalingCheckBox = row.add('checkbox', undefined, 'iOS');
		this.iosScalingCheckBox.value = this.iosScaling;
				
		this.ios2xScalingCheckBox = row.add('checkbox', undefined, 'iOS (@2x)');
		this.ios2xScalingCheckBox.value = this.ios2xScaling;
				
		this.mdpiScalingCheckBox = row.add('checkbox', undefined, 'mdpi');
		this.mdpiScalingCheckBox.value = this.mdpiScaling;

		this.hdpiScalingCheckBox = row.add('checkbox', undefined, 'hdpi');
		this.hdpiScalingCheckBox.value = this.hdpiScaling;
		
		this.xhdpiScalingCheckBox = row.add('checkbox', undefined, 'xhdpi');
		this.xhdpiScalingCheckBox.value = this.xhdpiScaling;
		...
		okBtn.onClick = function() { 
			...
			var documentName = multi_exporter.slug(docRef.name.replace('.ai',''));
			
			var base_path = multi_exporter.base_path;
			var scaling = multi_exporter.scaling;
			
			if (
				multi_exporter.iosScaling ||
				multi_exporter.ios2xScaling || 
				multi_exporter.mdpiScaling || 
				multi_exporter.hdpiScaling || 
				multi_exporter.xhdpiScaling
				) {
				
				if (multi_exporter.iosScaling) {
						multi_exporter.base_path = base_path + '/' + documentName;
						var myFolder = new Folder(multi_exporter.base_path);
						if(!myFolder.exists) myFolder.create();
						multi_exporter.scaling = multi_exporter._iosScaling;;
						multi_exporter.densitySuffix = false;
						try{
							if(multi_exporter.run_export() && !multi_exporter.ios2xScaling && !multi_exporter.mdpiScaling && !multi_exporter.hdpiScaling && !multi_exporter.xhdpiScaling) {
								multi_exporter.dlg.close();
							}
						} catch(e){
							alert(e);
				...
				if (multi_exporter.ios2xScaling) {
						multi_exporter.base_path = base_path + '/' + documentName;
						var myFolder = new Folder(multi_exporter.base_path);
						if(!myFolder.exists) myFolder.create();
						multi_exporter.scaling = multi_exporter._ios2xScaling;
						multi_exporter.densitySuffix = '@2x';
						
						try{
							if(multi_exporter.run_export() && !multi_exporter.mdpiScaling && !multi_exporter.hdpiScaling && !multi_exporter.xhdpiScaling) {
								multi_exporter.dlg.close();
							}
						} catch(e){
							alert(e);
				...
				if (multi_exporter.mdpiScaling) {
						multi_exporter.base_path = base_path + '/mdpi_' + documentName;
						var myFolder = new Folder(multi_exporter.base_path);
						if(!myFolder.exists) myFolder.create();
						multi_exporter.scaling = multi_exporter._mdpiScaling;
						multi_exporter.densitySuffix = false;
					
						try{
							if(multi_exporter.run_export() && !multi_exporter.hdpiScaling && !multi_exporter.xhdpiScaling) {
								multi_exporter.dlg.close();
							}
						} catch(e){
							alert(e);
				...
				if (multi_exporter.hdpiScaling) {
						multi_exporter.base_path = base_path + '/hdpi_' + documentName;
						var myFolder = new Folder(multi_exporter.base_path);
						if(!myFolder.exists) myFolder.create();
						multi_exporter.scaling = multi_exporter._hdpiScaling;
						multi_exporter.densitySuffix = false;
					
						try{
							if(multi_exporter.run_export() && !multi_exporter.xhdpiScaling) {
								multi_exporter.dlg.close();
							}
						} catch(e){
							alert(e);
				...
				if (multi_exporter.xhdpiScaling) {
						multi_exporter.base_path = base_path + '/xhdpi_' + documentName;
						var myFolder = new Folder(multi_exporter.base_path);
						if(!myFolder.exists) myFolder.create();
						multi_exporter.scaling = multi_exporter._xhdpiScaling;
						multi_exporter.densitySuffix = false;
					
						try{
							if(multi_exporter.run_export()) {
								multi_exporter.dlg.close();
							}
						} catch(e){
							alert(e);
		...
		// Devices format change handler
		
		var scalingCheckBoxFormat = function () {
			if (that.iosScalingCheckBox.value 
				|| that.ios2xScalingCheckBox.value
				|| that.mdpiScalingCheckBox.value
				|| that.hdpiScalingCheckBox.value
				|| that.xhdpiScalingCheckBox.value
			){
				that.scalingInput.enabled = false;
				that.export_devices = ((that.iosScalingCheckBox.value) ? 1:0) 
														+ ((that.ios2xScalingCheckBox.value) ? 1:0)
														+ ((that.mdpiScalingCheckBox.value) ? 1:0)
														+ ((that.hdpiScalingCheckBox.value) ? 1:0)
														+ ((that.xhdpiScalingCheckBox.value) ? 1:0);
				$.writeln('that.export_devices:' + that.export_devices);
			} else {
				that.scalingInput.enabled = true;
				that.export_devices = 1;
			}
			multi_exporter.update_export_desc();
		};
		
		this.iosScalingCheckBox.onClick = function() {
			scalingCheckBoxFormat();
		};
		
		this.ios2xScalingCheckBox.onClick = function() {
			scalingCheckBoxFormat();
		};
		
		this.mdpiScalingCheckBox.onClick = function() {
			scalingCheckBoxFormat();
		};
		
		this.hdpiScalingCheckBox.onClick = function() {
			scalingCheckBoxFormat();
		};
		
		this.xhdpiScalingCheckBox.onClick = function() {
			scalingCheckBoxFormat();
		};
	...
	updateExportCounts: function(){
		this.num_to_export = this.export_artboards.length * this.export_layers.length * this.export_devices;
    var artboardExportTxt;
		if(this.whole_artboard_mode){
			this.num_to_export += this.export_artboards.length * this.export_devices;
			if (this.export_layers.length){
				...
				// todo refactoring
				if (this.export_devices > 1) {
					this.progLabel.text = this.progLabel.text + ' (x' + this.export_devices + ')';
				}
    ...
    this.skipDefaultNames = this.skipDefaultNamesCheckBox.value;
		...
		this.iosScaling  = this.iosScalingCheckBox.value;
		this.ios2xScaling  = this.ios2xScalingCheckBox.value;
		this.mdpiScaling  = this.mdpiScalingCheckBox.value;
		this.hdpiScaling  = this.hdpiScalingCheckBox.value;
		this.xhdpiScaling  = this.xhdpiScalingCheckBox.value;
    ...
    this.prefs_xml.nyt_skipDefaultNames = this.skipDefaultNames;
		...
		this.prefs_xml.nyt_iosScaling  = this.iosScaling;
		this.prefs_xml.nyt_ios2xScaling  = this.ios2xScaling;
		this.prefs_xml.nyt_mdpiScaling  = this.mdpiScaling;
		this.prefs_xml.nyt_hdpiScaling  = this.hdpiScaling;
		this.prefs_xml.nyt_xhdpiScaling  = this.xhdpiScaling;
	...
	run_export: function() {
		...
		docRef.rasterEffectSettings.resolution = parseFloat(Math.round(this.scaling * 0.72));
    docRef.rasterEffectSettings.antiAliasing = true;

		for (var i = 0; i < this.export_artboards.length; i++ ) {
			var artboard = docRef.artboards[this.export_artboards[i]];
			var artboardName = artboard.name;

			starting_artboard = docRef.artboards.setActiveArtboardIndex(this.export_artboards[i]);
            ...
            copyDoc = this.copyDocument(docRef, artboard, rect, artW, artH, offset, 
              function(layer){
                return (layer.name != multi_exporter.PREFS_LAYER_NAME && layer.visible)
              }
            );
            formatInfo.saveFile(copyDoc, base_filename, options, this.export_artboards[i], artboardName, this.scaling, this.densitySuffix);
            copyDoc.close(SaveOptions.DONOTSAVECHANGES);
            copyDoc = null;
          } else {
            formatInfo.saveFile(docRef, base_filename, options, this.export_artboards[i], artboardName, this.scaling, this.densitySuffix);
          }
				} catch(e){
			...
			if(this.export_layers.length){
                ...
                suffix: this.suffix
							...
							if ( this.export_artboards.length == 1 ) {
							  base_filename.name = lyr_name;
								//base_filename = this.base_path + "/" + this.prefix + lyr_name + this.suffix;
							} else {
							  base_filename.name = artboardName + '-' + lyr_name;
